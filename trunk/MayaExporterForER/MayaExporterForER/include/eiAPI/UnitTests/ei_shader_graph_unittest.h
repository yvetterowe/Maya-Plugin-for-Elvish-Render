...
class ei_shader_graph_unittest : public CxxTest::TestSuite
{
public:
	void setUp();
	void tearDown();
...
private:
	void renderScene(const eiBool texturing, const char *suffix);
};

void ei_shader_graph_unittest::setUp()
{
}

void ei_shader_graph_unittest::tearDown()
{
}

void ei_shader_graph_unittest::testBasic()
{
	renderScene(eiFALSE, "basic");
}

void ei_shader_graph_unittest::testTexture()
{
	renderScene(eiTRUE, "texture");
}

void ei_shader_graph_unittest::renderScene(const eiBool texturing, const char *suffix)
{
	...
	ei_camera("cam1");
		sprintf(filename, "frame_shader_graph_%s02.bmp", suffix);
		ei_append_filename(output_filename, cur_dir, filename);
		ei_output(output_filename, "bmp", EI_IMG_DATA_RGB);
			ei_output_variable("color", EI_DATA_TYPE_VECTOR);
		ei_end_output();
