...
#define DECLARE_POINT(param_name, x, y, z) \
	if (pid >= 0) {\
		E_##param_name = pid++;\
	} else {\
		ei_declare_shader_param_vector(#param_name, x, y, z);\
...
#define DECLARE_NORMAL(param_name, x, y, z) \
	if (pid >= 0) {\
		E_##param_name = pid++;\
	} else {\
		ei_declare_shader_param_vector(#param_name, x, y, z);\
...
#define DECLARE_COLOR(param_name, x, y, z) \
	if (pid >= 0) {\
		E_##param_name = pid++;\
	} else {\
		ei_declare_shader_param_vector(#param_name, x, y, z);\
...
#define PARAM(T, name) \
	...
	\
	T& Duv_##name(T& dXdu, T& dXdv)\
	{\
		return *((T *)ei_call_uv(get_state(), E_##name, (eiByte * const)(&dXdu), (eiByte * const)(&dXdv)));\
	}\
	\
	T& Dxy_##name(T& dXdx, T& dXdy)\
	{\
		return *((T *)ei_call_xy(get_state(), E_##name, (eiByte * const)(&dXdx), (eiByte * const)(&dXdy)));\
	}

#define Duv(name, dXdu, dXdv) Duv_##name(dXdu, dXdv)
#define Dxy(name, dXdx, dXdy) Dxy_##name(dXdx, dXdy)
...
protected:
	...
	inline vector & dPdx()
	{
		return *((vector *)(&get_state()->dPdx));
	}

	inline vector & dPdy()
	{
		return *((vector *)(&get_state()->dPdy));
	}
	...
	inline scalar & Dxy_u(scalar & dudx, scalar & dudy)
	{
		dudx = get_state()->dudx;
		dudy = get_state()->dudy;
		return get_state()->u;
	...
	inline scalar & Dxy_v(scalar & dvdx, scalar & dvdy)
	{
		dvdx = get_state()->dvdx;
		dvdy = get_state()->dvdy;
		return get_state()->v;
	...
	inline eiBool call_shader(
		...
		void *arg)
	{
		return ei_call_shader(result, get_state(), shader, arg);
	}
