...
{
	return ei_eval_imp(state, param_index, eiTRUE, NULL, NULL);
}

void *ei_call(eiState *state, const eiIndex param_index)
{
	return ei_eval_imp(state, param_index, eiFALSE, NULL, NULL);
}

void *ei_call_uv(eiState *state, const eiIndex param_index, eiByte * const dXdu, eiByte * const dXdv)
{
	return ei_eval_imp(state, param_index, eiFALSE, dXdu, dXdv);
}

void *ei_call_xy(eiState *state, const eiIndex param_index, eiByte * const dXdx, eiByte * const dXdy)
{
	return ei_eval_imp(state, param_index, eiFALSE, dXdx, dXdy);
}
...
static eiFORCEINLINE eiScalar CosPhi(const eiVector *w)
{
    ...
    sintheta = SinTheta(w);
	if (almost_zero(sintheta, BSDF_EPS)) {
		return 1.0f;
	}
	result = w->x / sintheta;
...
static eiFORCEINLINE void SchlickFresnel(
	...
	const eiVector *Rs)
{
	eiScalar k = powf(MAX(BSDF_EPS, 1.0f - costheta), 5.0f);
	...
	void *dist_param)
{
	...
	Kd = (28.0f / (23.0f * (eiScalar)eiPI)) * 
		(1.0f - powf(MAX(BSDF_EPS, 1.0f - 0.5f * AbsCosTheta(wi)), 5.0f)) * 
		(1.0f - powf(MAX(BSDF_EPS, 1.0f - 0.5f * AbsCosTheta(wo)), 5.0f));
	...
	invKs = (8.0f * (eiScalar)eiPI * absf(dot(wi, &wh)) * MAX(AbsCosTheta(wi), AbsCosTheta(wo)));
	if (almost_zero(invKs, BSDF_EPS))
	{
		setvf(f, 0.0f);
		return;
	}
	/* premultiply cosThetaI, is this correct? */
	Ks = D * AbsCosTheta(wi) / invKs;
	specular.r = Ks * F.r;
	specular.g = Ks * F.g;
	specular.b = Ks * F.b;
...
eiScalar ei_fresnelblend_pdf(
	...
	void *dist_param)
{
	return 0.5f * (MAX(BSDF_EPS, AbsCosTheta(wi)) * (1.0f / (eiScalar)eiPI) + dist(wo, wi, dist_param));
}
...
void ei_lambert_bsdf(
	...
	const eiVector *wi)
{
	/* premultiply by cosThetaI */
	setvf(f, AbsCosTheta(wi) * (1.0f / (eiScalar)eiPI));
}
...
eiScalar ei_lambert_pdf(
	...
	const eiVector *wi)
{
	return SameHemisphere(wo, wi) ? MAX(BSDF_EPS, AbsCosTheta(wi)) * (1.0f / (eiScalar)eiPI) : 0.0f;
}
	...
	const eiScalar sig)
{
    ...
    maxcos = 0.0f;
    if (sinthetai > BSDF_EPS && sinthetao > BSDF_EPS) {
	...
	/* premultiply by cosThetaI */
	setvf(f, AbsCosTheta(wi) * (1.0f / (eiScalar)eiPI) * (A + B * maxcos * sinalpha * tanbeta));
}
...
void ei_specularreflection_bsdf(
	...
	const eiVector *wi)
{
	/* we don't need to divide cosThetaI because we premultiply it */
	setvf(f, 1.0f);
}
	...
	const eiVector *wh)
{
	...
	if (almost_zero(WOdotWh, BSDF_EPS))
	{
		return 0.0f;
	}
	...
	const eiVector *wi)
{
	...
	if (almost_zero(cosThetaI, BSDF_EPS) || almost_zero(cosThetaO, BSDF_EPS)) {
		setvf(result, 0.0f);
		return;
	}
	add(&wh, wi, wo);
	...
	normalizei(&wh);
	if (almost_zero(4.0f * cosThetaO, BSDF_EPS))
	{
		setvf(result, 0.0f);
		return;
	}
	/* we don't need to divide cosThetaI here because we premultiply it */
    setvf(result, G(wo, wi, &wh) / (4.0f * cosThetaO));
}
	...
	void *param)
{
	...
	costhetah = MAX(BSDF_EPS, AbsCosTheta(wh));
	...
	void *param)
{
	...
	costheta = MAX(BSDF_EPS, AbsCosTheta(&wh));
	pdf = eiSCALAR_EPS;
	if (dot(wo, &wh) > BSDF_EPS) {
	...
	eiScalar *phi, eiScalar *costheta)
{
	...
	cosphi = cosf(*phi);
	sinphi = sinf(*phi);
	*costheta = powf(MAX(BSDF_EPS, u2), 1.0f / (shiny_u * cosphi * cosphi + shiny_v * sinphi * sinphi + 1.0f));
}
	...
	void *param)
{
    ...
    costhetah = MAX(BSDF_EPS, AbsCosTheta(&wh));
    ds = 1.0f - costhetah * costhetah;
    pdf = BSDF_EPS;
    if (ds > BSDF_EPS && dot(wo, &wh) > BSDF_EPS) {
		eiScalar	e, d;
