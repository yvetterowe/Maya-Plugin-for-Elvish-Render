	...
	eiByte * const dXdv)
{
	...
	/* disable shader cache if we are calculating derivatives, 
	   because currently we don't cache derivatives, in order 
	   to save cache memory */
	if (dXdu != NULL || dXdv != NULL)
	{
		shader_cache_enabled = eiFALSE;
	}
	...
	/* zero derivatives, because derivatives should be zero for constant 
	   parameters, and for connected parameters, currently we cannot 
	   compute derivatives for them */
	if (dXdu != NULL)
	{
		memset(dXdu, 0, shader_param->size);
	}
	if (dXdv != NULL)
	{
		memset(dXdv, 0, shader_param->size);
	}
	...
	else
	{
		...
		if (dXdu != NULL || dXdv != NULL)
		{
			dX1 = (eiByte *)_alloca(shader_param->size * 2);
			dX2 = dX1 + shader_param->size;
		}
			...
			dX2))
		{
			if (dXdu != NULL)
			{
				deriv_u_any(dXdu, dX1, dX2, state, shader_param->type);
			}
			if (dXdv != NULL)
			{
				deriv_v_any(dXdv, dX1, dX2, state, shader_param->type);
			}
		}
		else
		{
