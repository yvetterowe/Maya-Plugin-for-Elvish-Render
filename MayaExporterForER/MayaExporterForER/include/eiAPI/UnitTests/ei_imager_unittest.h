...
class ei_imager_unittest : public CxxTest::TestSuite
{
public:
	void setUp();
	void tearDown();
...
void ei_imager_unittest::setUp()
{
}

void ei_imager_unittest::tearDown()
{
}

void ei_imager_unittest::testOverall()
{
	...
	ei_shader("imager_shader");
		ei_shader_param_string("desc", "simple_imager");
		ei_shader_param_scalar("brightness", 1.8f);
	ei_end_shader();

	ei_camera("cam1");
		ei_append_filename(output_filename, cur_dir, "frame_imager01.bmp");
	...
	ei_camera("cam1");
		ei_append_filename(output_filename, cur_dir, "frame_imager02.bmp");
		ei_output(output_filename, "bmp", EI_IMG_DATA_RGB);
			ei_output_variable("color", EI_DATA_TYPE_VECTOR);
		ei_end_output();
		ei_aperture(100.0f);
