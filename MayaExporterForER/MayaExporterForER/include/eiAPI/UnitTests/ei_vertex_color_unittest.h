...
class ei_vertex_color_unittest : public CxxTest::TestSuite
{
public:
	void setUp();
	void tearDown();
	...
	void testBasic();
	void testTessellated();

private:
	void renderApprox(const eiApprox *approx, const char *suffix);
};

void ei_vertex_color_unittest::setUp()
{
}

void ei_vertex_color_unittest::tearDown()
{
}

void ei_vertex_color_unittest::testBasic()
{
	renderApprox(NULL, "basic");
}

void ei_vertex_color_unittest::testTessellated()
{
	eiApprox	approx;
	...
	renderApprox(&approx, "tessellated");
}

void ei_vertex_color_unittest::renderApprox(const eiApprox *approx, const char *suffix)
{
	...
	ei_camera("cam1");
		sprintf(filename, "frame_vertex_color_%s01.bmp", suffix);
	...
	ei_object("obj1", "poly");
		...
		ei_declare("Cs", eiVARYING, EI_DATA_TYPE_TAG, &Cs_tab);
	...
	ei_camera("cam1");
		sprintf(filename, "frame_vertex_color_%s02.bmp", suffix);
		ei_append_filename(output_filename, cur_dir, filename);
		ei_output(output_filename, "bmp", EI_IMG_DATA_RGB);
			ei_output_variable("color", EI_DATA_TYPE_VECTOR);
		ei_end_output();
