...
class ei_dof_unittest : public CxxTest::TestSuite
{
public:
	void setUp();
	void tearDown();
...
void ei_dof_unittest::setUp()
{
}

void ei_dof_unittest::tearDown()
{
}

void ei_dof_unittest::testOverall()
{
	...
	ei_shader("dof_shader");
		...
		ei_shader_param_scalar("fplane", 0.1f);
	ei_end_shader();

	ei_camera("cam1");
		ei_append_filename(output_filename, cur_dir, "frame_dof01.bmp");
	...
	ei_camera("cam1");
		ei_append_filename(output_filename, cur_dir, "frame_dof02.bmp");
		ei_output(output_filename, "bmp", EI_IMG_DATA_RGB);
			ei_output_variable("color", EI_DATA_TYPE_VECTOR);
		ei_end_output();
		ei_aperture(100.0f);
